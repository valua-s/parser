...
def search_products(query, region_name=''):
    q_set = query.split(' ')
    q_request = q_set[0]
    for q in q_set[1:]:
        q_request += f'+{q}'
...
def parse_product(product_url: str, region_name):
    if region_name:
        region_name += '.'
...
def main(queries: dict, region_names: dict):
    all_products = []
    
    for query in queries:
        for region_name in region_names:
            product_links = search_products(query, region_name)
        
            for product_link in product_links:
                product_data = parse_product(product_link, region_name)
